...
//                        }
        ...
        .onChange(of: dataManager.isDarkMode) { _ in
        }
        .onAppear {
            dataManager.isDarkMode = (colorScheme == .light) // Set the initial state based on color scheme
        }
...
func darkModeView(colorScheme: ColorScheme, dataManager: DataManager) -> some View {
    VStack {
        if colorScheme == .light {
            // This is the view that appears when in light mode
            EmptyView()
        } else {
            // This is the view when in dark mode or if you want specific logic without symbols
            EmptyView()
        }
    }
    .environment(\.colorScheme, dataManager.isDarkMode ? .dark : .light)
    .onAppear {
        dataManager.isDarkMode = (colorScheme == .light)
    }
    .onChange(of: dataManager.isDarkMode) { newValue in
        if let windowScene = UIApplication.shared.connectedScenes.first as? UIWindowScene,
           let window = windowScene.windows.first {
